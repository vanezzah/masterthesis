...
```{r}
fit_multinom <- multinom(numcars ~ numlic + I(numlic^2) + region2 + income_numerical+ quali_nv + CSyes + CSmultiple + workers +triplength_avg+ I(triplength_avg^2) + log(nummots1)+ housing_type + quali_opnv + share2039 + share4064 + hh_children + garage + metro28 + train28 + tripsavg + parttime + numped + bus28, data = HH_dt_2_out)
...
figure_incnum <- ggarrange(numlic_plot, income_plot,common.legend = TRUE, legend = "right", 
          labels = c("License holders", "Income numerical"), vjust=10,
          ncol = 2, nrow = 1)
...
figure_trips <- ggarrange(triplength_plot, trips_plot,common.legend = TRUE, legend = "right", 
          labels = c("Av. triplength", "Av. trips"), vjust=10,
          ncol = 2, nrow = 1)
...
figure_trips <- ggarrange(workers_plot, parttime_plot,common.legend = TRUE, legend = "right", 
          labels = c("Number of workers", "Number of parttime workers"), vjust=6,
          ncol = 2, nrow = 1)
...
share2_plot <-ggplot(data=e13, aes(x=share4064))+
      geom_line(aes(y=prob.X0, color="Zero cars")) + 
      geom_line(aes(y=prob.X1, color="One car")) + 
      geom_line(aes(y=prob.X2, color ="Two cars"))+
      geom_line(aes(y=prob.X3, color ="Three cars"))+
...
figure_chidregion<- ggarrange(children_plot, region_plot,common.legend = TRUE, legend = "right", 
          labels = c("Children", "Region"), vjust=6,
          ncol = 2, nrow = 1)
...
figure_pedmots<- ggarrange(numped_plot, nummots_plot,common.legend = TRUE, legend = "right", 
          labels = c("Bicycle", "Motorbike"), vjust=6,
          ncol = 2, nrow = 1)
...
train_plot <-ggplot(data=e19, aes(x=train28, group=1))+
      ...
      coord_fixed(ratio=3)+
      scale_y_continuous(limits=c(0,1),breaks = c(0.2,0.4,0.6,0.8))+
      scale_x_discrete(labels = c("middle"="medium"))+
      theme_bw()+ 
        theme(panel.grid.major=element_blank(),
...
figure_bus<- ggarrange(bus_plot, metro_plot,train_plot,common.legend = TRUE, legend = "right", 
          labels = c("bus", "metro", "train"), vjust=10,
          ncol = 3, nrow = 1)
...
figure_housing<- ggarrange(housing_plot, garage_plot,common.legend = TRUE, legend = "right", 
          labels = c("Housing type", "Garage"), vjust=6,
          ncol = 2, nrow = 1)
