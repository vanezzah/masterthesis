...
# hhsize - numerical, fine like it is
...
# as ordered factor: 
HH_dt_model1$einkommen <- factor(HH_dt_model1$einkommen, ordered = FALSE)
...
#temp_region <- HH_dt_model1[, "region"]
...
# as ordered factor: 
HH_dt_model1$bus28 <- factor(HH_dt_model1$bus28)
...
# as ordered factor: 
HH_dt_model1$bahn28 <- factor(HH_dt_model1$bahn28, ordered = TRUE)
...
# as ordered factor: 
HH_dt_model1$tram28 <- factor(HH_dt_model1$tram28, ordered = TRUE)
...
HH_dt_model1$quali_nv<- factor(HH_dt_model1$quali_nv, ordered = FALSE)

#quali_opnv, ordered factor
dplyr::count(HH_dt_model1, quali_opnv)
HH_dt_model1$quali_opnv<- factor(HH_dt_model1$quali_opnv, ordered = FALSE)
...
```{r}
#anzahlen raus, quali_öpnv raus, collapse categories of einkommen and where people live
...
model1 <- mlogit(anzauto ~ 1 | quali_opnv | 1, data = datamlogit, reflevel = "0")
...
#constrained model: 
model2 <- mlogit(anzauto ~ 1 | hheink_gr2 | 1, data = datamlogit, reflevel = "0")
...
test <- multinom(anzauto2 ~ hheink_gr2 + Weglänge_avg, data = HH_test)
...
fm3 <- formula(anzauto ~ 1 | anzfs| 1)
fit3 <- mnlogit(fm3, mlogitSample, ncores=2, choiceVar = "alt")

fm <- formula(anzauto ~ 1 | anzfs + region + einkommen + quali_nv + householdtype + berufstaetige + CS + Weglänge_avg + anzmots + anzahl2039 + haustyp + quali_opnv + anzahl65 + Bildung| 1)
...
library(car)
x_reg <- lm(anzauto ~ anzfs + region + einkommen + quali_nv + berufstaetige + CS + householdtype + Weglänge_avg + anzmots + anzahl2039 + haustyp + quali_opnv + anzahl65 + Bildung + bahn28, data = HH_dt_model1)
vif(x_reg)
...
b <- c(0,B[c("einkommen2:1", "einkommen2:2", "einkommen2:3")])
pb <- probs[,2] * b[2]  + probs[,3] * b[3] + probs[,4] * b[4]
me <- matrix(0, nrow(probs), ncol(probs))
for(j in 1:4) {
  me[,j] <- probs[,j] * (b[j] - pb)
...
Regression using VGAM
```{r}

install.packages("VGAM")
library(VGAM)

vglmFitMN <- vglm(anzauto_gr2 ~ hheink_gr2 + Weglänge_avg, family=multinomial(refLevel=1), HH_test)
...
HH_dt <-copy_to(sc, HH_dt_model1)


HH_dt %>% 
  ml_logistic_regression(anzauto ~ einkommen, family= "multinomial") %>%
  summary()
