...
# import all categorical columns (alle ausser nuermische variablen)
...
# numerische variable weglänge_avg mit allen kategoriellen variablen die nicht ordinal sind (e.g. CS, bildung, RegioStarGem5, garage, haustyp, single - couple )
...
# transform dt
datamlogit <- mlogit.data(HH_test2, choice = "anzauto_gr2", shape = "wide")

model1 <- mlogit(anzauto_gr2 ~ 1 | hheink_gr2 | 1, data = datamlogit, reflevel = "0")
...
#constrained model: 
model2 <- mlogit(anzauto_gr2 ~ 1 | hheink_gr2 | 1, data = datamlogit, reflevel = "0")
...
HH_test2 <- HH_test[, -c(4)]

# needs data in long format (like mlogit)
mlogitSample <- mlogit.data(HH_test2, choice = "anzauto_gr2", shape = "wide") 
head(mlogitSample)
names(mlogitSample)[names(mlogitSample) == 'anzauto_gr2'] <- "anzautos"
names(mlogitSample)[names(mlogitSample) == 'hheink_gr2'] <- "hheink"
names(mlogitSample)[names(mlogitSample) == 'Weglänge_avg'] <- "weglängeavg"
...
fm <- formula(anzautos ~ 1 | hheink + weglängeavg | 1)
fm2 <- formula(anzautos ~ 1 | hheink | 1)
fit <- mnlogit(fm, mlogitSample, ncores=2, choiceVar = "alt")
