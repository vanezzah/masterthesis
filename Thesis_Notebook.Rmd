...
# Aufteilung in share2039 share4064 share65
...
for (row in rows){
  for (col in cols) {
      if (alter_dt[row, ..col] < 40 & alter_dt[row, ..col] > 19) {
    temp2039[row] <- temp2039[row] + 1 }
    else if (alter_dt[row, ..col] < 65 & alter_dt[row, ..col] > 39){
     temp4064[row] <- temp4064[row] + 1 }
    else if (alter_dt[row, ..col] > 64){
     temp65[row] <- temp65[row] + 1 }
...
#faster? :
# für 20-39 jährige:
temp <- alter_dt[1:20,2:9]

result <- temp < 40 & test > 19
result <- as.data.table(result)
result <- result*1
result
anzahl2029 <- apply(result,1,sum)
anzahl2029 / alter_dt[1:20,1]

# noch anhängen und für andere machen
```
