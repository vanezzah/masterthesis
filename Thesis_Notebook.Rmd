...
fm3 <- formula(anzauto ~ 1 | anzfs + region + hheink_imp + quali_nv + berufstaetige + householdtype+ CS + Weglänge_avg + anzmots + anzahl2039 + haustyp + quali_opnv + anzahl65 + Bildung + bahn28 + garage + anzahlWege + region*anzfs + region:quali_nv| 1)
fit3 <- mnlogit(fm3, mlogitSample2, ncores=4, choiceVar = "alt")

install.packages("effects")
library(effects)

regionanzfs.effect <- effect( "region*anzfs", mlogitSample2,xlevels = list(anzfs = 1:11,   region = 51:55))

fm <- formula(anzauto ~ 1 | anzfs + region + einkommen + quali_nv + berufstaetige + householdtype+ CS + Weglänge_avg + anzmots + anzahl2039 + haustyp + quali_opnv + anzahl65 + Bildung + bahn28 + garage + anzahlWege| 1)
...
x_reg <- lm(anzauto ~ anzfs + region + einkommen + quali_nv + berufstaetige + CS + householdtype + Weglänge_avg + anzmots + anzahl2039 + haustyp + quali_opnv + anzahl65 + Bildung + bahn28 + garage + anzahlWege, data = HH_dt_model1)
vif(x_reg)
...
as.matrix(B)
b <- c(0,B[c("region52:1", "region52:2", "region52:3")])
...
fit_mlogit <- mlogit(anzauto ~ 1 | anzfs + region + einkommen + quali_nv + berufstaetige + householdtype+ CS + Weglänge_avg + anzmots + anzahl2039 + haustyp + quali_opnv + anzahl65 + Bildung + bahn28 + garage + anzahlWege| 1, data = mlogitSample, reflevel = "0")


c.names <- names(fit_mlogit$model)[- c(1, 19:21)]

ME.mnl <- sapply(c.names, function(x) 
  stats::effects(fit_mlogit, covariate=x, data=mlogitSample), 
  simplify=FALSE) 

Me_test <-  stats::effects(fit_mlogit, covariate="region52", data=mlogitSample)
...
myeffects(fit_mlogit, covariate = "region52", data = mlogitSample)
```
