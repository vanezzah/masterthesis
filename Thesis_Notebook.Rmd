...
numerical_dt <- HH_dt_final[, c("anzahlWege","perskm2", "H_ANZMOTMOP", "anzpedrad", "HP_ANZFS", "anzkind18", "berufstaetige", "share4064", "share65","share2039")]
...
# durchschnittle wegelänge statt perskm2?

singlehhsize <- HH_dt_final[, c("single", "perskm2")]
singlehhsize <- filter(singlehhsize, perskm2 < 5000 )
library(ggplot2)
beanplot(perskm2~ single, data=HH_dt_final)
summary(singlehhsize)

#check if its normally distributed
ggplot(singlehhsize, aes(x=single, y=perskm2)) + 
  geom_violin() + facet_wrap(~single)

#weitere wege cutten?

# shapiro test
single1 <- filter(singlehhsize, single == 1)

shapiro.test(singlehhsize$perskm2)
```
