...
predict(fit_multitest)
pred_accuracy = mean(predict(fit_multitest)==HH_dt_model2$anzauto)
pred_accuracy

logitgof(HH_dt_model2$anzauto, fitted(fit_multitest), g=10, ord=FALSE)

install.packages('rms')
library(rms)
mod1b <- lrm(anzauto ~ anzfs + region + hheink_imp+ quali_nv + berufstaetige + householdtype + CS, data = HH_dt_model2)
...
HH_dt_2_bt <- HH_dt_2
HH_dt_2_bt$numcars <- HH_dt_2$numcars
HH_dt_2_bt$trips <- HH_dt_2$trips + 1
HH_dt_2_bt$triplength_avg <- HH_dt_2$triplength_avg + 1
HH_dt_2_bt$nummots <- HH_dt_2$nummots + 1
HH_dt_2_bt$numped <- HH_dt_2$numped + 1
HH_dt_2_bt$numlic <- HH_dt_2$numlic + 1
HH_dt_2_bt$income_numerical <- HH_dt_2$income_numerical + 1
HH_dt_2_bt$numch18 <- HH_dt_2$numch18 + 1
HH_dt_2_bt$workers <- HH_dt_2$workers + 1
HH_dt_2_bt$num2039 <- HH_dt_2$num2039+ 1
HH_dt_2_bt$num4064 <- HH_dt_2$num4064+ 1
HH_dt_2_bt$num65 <- HH_dt_2$num65+ 1

HH_dt_2_bt <- HH_dt_2_out
HH_dt_2_bt$numcars <- HH_dt_2_out$numcars
HH_dt_2_bt$trips <- HH_dt_2_out$trips + 1
HH_dt_2_bt$triplength_avg <- HH_dt_2_out$triplength_avg + 1
HH_dt_2_bt$nummots <- HH_dt_2_out$nummots + 1
HH_dt_2_bt$numped <- HH_dt_2_out$numped + 1
HH_dt_2_bt$numlic <- HH_dt_2_out$numlic + 1
HH_dt_2_bt$income_numerical <- HH_dt_2_out$income_numerical + 1
HH_dt_2_bt$numch18 <- HH_dt_2_out$numch18 + 1
HH_dt_2_bt$workers <- HH_dt_2_out$workers + 1
HH_dt_2_bt$num2039 <- HH_dt_2_out$num2039+ 1
HH_dt_2_bt$num4064 <- HH_dt_2_out$num4064+ 1
HH_dt_2_bt$num65 <- HH_dt_2_out$num65+ 1



#in mnlogit
boxTidwell(numcars ~  triplength_avg + income_numerical  , ~ education + CS + region2 + bus28 + train28 + metro28 + housing_type + garage + quali_opnv + quali_nv + householdtype_all , data=HH_dt_2_bt)

#triplength raus
# num4064 raus
#numlic raus

boxTidwell(numcars ~  triplength_avg + income_numerical  , ~ trips + numch18 + workers + num2039 + num65 + education + CS + + nummots+ numped +region2 + bus28 + train28 + metro28 + housing_type + garage + quali_opnv + quali_nv + householdtype_all , data=HH_dt_2_bt)

#transform into long format
HH_dt_2_bt$log.triplength  <- log(HH_dt_2_bt$triplength_avg)
HH_dt_2_bt$log.triplength <- HH_dt_2_bt$log.triplength - mean(HH_dt_2_bt$log.triplength)



mlogit_testlin <- mlogit.data(HH_dt_2_bt, choice = "numcars", shape = "wide") 

#run model to see if linearity holds
fm_testlin <- formula(numcars ~ 1 |  trips + trips:log(trips) + nummots + nummots:log(nummots) + numped + numped:log(numped) + numlic + numlic:log(numlic) + numch18 + numch18:log(numch18) + workers + workers:log(workers) + num2039 +  num2039:log(num2039) + num65 + num65:log(num65) + education + CS + region2 + bus28 + train28 + metro28 + housing_type + garage + quali_opnv + quali_nv + householdtype_all| 1)
fit_testlin <- mnlogit(fm_testlin, mlogit_testlin, ncores=4, choiceVar = "alt", verbose = T)
summary(fit_testlin)

#only real continuous variables 
fm_testlin <- formula(numcars ~ 1 | log(income_numerical) + log.triplength  + trips +trips*trips + nummots  + numped + numlic + numch18  + log(workers) + num2039 +  num65 + education + CS + region2 + bus28 + train28 + metro28 + housing_type + garage + quali_opnv + quali_nv + householdtype_all| 1)
fit_testlin <- mnlogit(fm_testlin, mlogit_testlin, ncores=4, choiceVar = "alt", verbose = T)
summary(fit_testlin)

x1 <- predict(fit_testlin)
x2 <- x1[,2]
y <- resid(fit_testlin, type="response")
library(arm)
binnedplot(x2,y2)
...
#ausprobieren mit numlogit

coefs <- as.data.frame(fit_testlin$coefficients)
```
