...
```{r}
res <- wilcox.test(merural[,1]	,mesuburban[,1], paired = TRUE)
res
```
...
#Average marginal effects for interaction effects

```{r}
B <- coef(fit_int)
probs <- predict(fit_int, type="probs")
#B <- as.matrix(B)

b <- c(0,B[c("region2suburban:numlic:1", "region2suburban:numlic:2", "region2suburban:numlic:3")])
pb <- probs[,2] * b[2]  + probs[,3] * b[3] + probs[,4] * b[4]
me <- matrix(0, nrow(probs), ncol(probs))
for(j in 1:4) {
  me[,j] <- probs[,j] * (b[j] - pb)
...
b <- c(0,B[c("region2rural:numlic:1", "region2rural:numlic:2", "region2rural:numlic:3")])
pb <- probs[,2] * b[2]  + probs[,3] * b[3] + probs[,4] * b[4]
me <- matrix(0, nrow(probs), ncol(probs))
for(j in 1:4) {
  me[,j] <- probs[,j] * (b[j] - pb)
