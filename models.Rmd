...
model2 <- mlogit(numcars ~ 1, data = mnlogit_dt_out)
...
print(xtable(AME_df, type = "latex",digits=c(0,0,3,3,3,3)),include.rownames=FALSE, file = "AME.tex")
...
summary(margins(data=data.frame(HH_dt_2_out), fit_multinom2, variables = "CSyes",category="0"))
...
c.names <- names(fit_new$model)[- c(1, 26:28)]

# get marginal effects
ME.mnl <- sapply(c.names, function(x) 
  stats::effects(fit_multinom_v3, covariate=x, data=HH_dt_2_out), 
  simplify=FALSE) 

# get AMEs
(AME.mnl <- t(sapply(ME.mnl, colMeans)))

c.names <- names(fit_int$model)[- c(1, 29:31)]
...
c.names <- names(fit_new$model)[- c(1, 26:28)]
c.names <- fit_multinom_v3$coefnames[-c(1)]

AME.fun <- function(betas) {
  tmp <- fit_multinom_v3
  tmp$coefficients <- betas
  ME.mnl <- sapply(names_numint, function(x) 
    marginal_effects(tmp, variables = x, data = HH_dt_2_out, category = "0"), simplify = FALSE)
  c(sapply(ME.mnl, colMeans))
}

require(numDeriv)
grad <- jacobian(AME.fun, coef(fit_multinom_v3))

(AME.mnl.se <- matrix(sqrt(diag(grad %*% vcov(fit_multinom_v3) %*% t(grad))), nrow = 3, byrow = TRUE))
...
library(margins)
test_margins_numlic <- margins(multinom_int3, variables="numlic", data = data.frame(HH_dt_2_out), category = "1")

cplot(test_margins1, "numlic")
